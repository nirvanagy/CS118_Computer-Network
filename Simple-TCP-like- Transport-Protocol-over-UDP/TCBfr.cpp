...
void TCB::send() {
    ...
    while (true) {
        ...
        newACK=false;
        if (len < 0) {
            update_windows_size(true, retrByTimer,newACK);
            dupAckCount=0;
            if (!buffer.empty()) {
            ...
            if (recv_packet.header.ackNo == seqNo) {
                dupAckCount++;
                update_windows_size(false, retrByTimer,newACK);
                if (dupAckCount == 3) {
                    if (send_packet(buffer.front()->packet, buffer.front()->len) < 0) {
...
void TCB::update_windows_size(bool timeout, bool retrByTimer,bool newACK){
    uint16_t curWin;
    if(timeout){
