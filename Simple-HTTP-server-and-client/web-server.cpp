...
{
    std::cout<<"new thread for "<<client_fd<<std::endl;
    std::string instring;
    char inbuf[BUFSIZE];
    long recvLen;
    ...
    while (true)
    {
        ...
        while (instring.find("\r\n\r\n") == std::string::npos)
        {
            ...
            if(recvLen==0){
                printf("Recv error: Incomplete request\n");
                exit(1);
            }
        ...
        //If http1.1, keep connection for SOME time
       
        /*if(server_req.getVersion().compare("HTTP/1.1")==0)
        {
            server_res.setHeader("Connection", "Keep-Alive");
            std::string value="timeout="+std::to_string(TIMEALIVE)+", max=100";
            server_res.setHeader("Keep-Alive", value);
        }
        else */
        ...
        if(server_req.getVersion().compare("HTTP/1.1")!=0)
        {
            std::cout<<"Http 1.0: Data Send. Connection closed. Thread:"<<client_fd<<std::endl;
            close(client_fd);
            return;
        }
        //http 1.1 to be implemented
    }
}
