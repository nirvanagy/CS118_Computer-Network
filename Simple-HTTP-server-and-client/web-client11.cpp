...
{
    map<string,int>reqPerConnection;
    //Parse all URLs
    if(argc == 1){
    ...
    test_argv.push_back("http://localhost:4000/CS118-pj1/test_Makefile");
    test_argv.push_back("http://localhost:4000/CS118-pj1/test_Makefile");
    test_argv.push_back("http://localhost:4000/CS118-pj1/test_Makefile");
    test_argv.push_back("http://localhost:4000/CS118-pj1/test_Makefile");*/
    ...
    for(int i=1; i<=n_url; i++){
        //url[i-1] = parseUrl(test_argv[i-1]);
        ...
        if(reqPerConnection.find(url[i-1].host+url[i-1].port)==reqPerConnection.end()){
            reqPerConnection[url[i-1].host+url[i-1].port]=1;
        }
        else  reqPerConnection[url[i-1].host+url[i-1].port]++;
    }
    ...
    for(int i=0; i<n_url; i++){
        ...
        if (p == NULL) {
            fprintf(stderr, "connect error: failed to connect\n");
            exit(1);
        }
        ...
        clt_req.setVersion("HTTP/1.1");
        clt_req.setHeader("Host", url[i].host);
        if(reqPerConnection[url[i].host+url[i].port]==1){
            clt_req.setHeader("Connection","close");
        }
        else clt_req.setHeader("Connection","keep-alive");
        ...
        reqPerConnection[url[i].host+url[i].port]--;
        std::cerr<<"Connection:"<<url[i].host+url[i].port<<" Request time:"<<reqPerConnection[url[i].host+url[i].port]<<std::endl;
        //Read Response
        ...
        //Close connection
        if(clt_res.getHeaderValue("Connection").compare("close")==0){
            close(sockfd);
        }
        // Store the file
        if(clt_res.getStatus()==404||clt_res.getStatus()==400)
            ...
            std::cerr<<"file name is: "<<filename<<endl;
            file.open(filename);
